...
CREATE = """INSERT INTO Usuarios (nombreUsuario, Nombre, email, fechaNacimiento, DNI, Rol, password, telefono)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)"""
UPDATE = """UPDATE Usuarios
            SET Nombre = ?, email = ?, DNI = ?, password = ?, fechaNacimiento = ?, telefono = ?
            WHERE nombreUsuario = ?"""
...
class UsuarioDAO:
    ...
    def get_by_email(email):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute(GET_BY_EMAIL, (email,))
    ...
    def get_all():
        ...
        try:
            ...
            for raw_row in raw_rows:
                ...
                val_activo = row.get('activo') or row.get('Activo')
                ...
                if rol == 'paciente':
                    ...
                    tipo_val = (
                        row.get('TipoTrabajador') or 
                        row.get('tipotrabajador') or
                        row.get('TipoPaciente') or
                        row.get('tipopaciente') or 
                        ''
                    )
                    row['Tipo'] = tipo_val
